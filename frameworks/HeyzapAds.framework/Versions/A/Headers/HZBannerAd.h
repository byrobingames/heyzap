...
/**
 *  @brief Called when a banner ad loads or refreshes itself with a new "fill".
 *
 *  @param banner A banner object that has loaded an ad.
 */
- (void)bannerDidReceiveAd:(HZBannerAd *)banner;

/**
 *  @brief Called when a banner ad has been shown or it is already shown and it has refreshed itself with a new "fill".
 *
 *  @param banner A banner object that has shown an ad.
 *  @param adImpressionData The ad pricing details about the new "fill" (including the eCPM).
 */
- (void)bannerDidShowAd:(HZBannerAd *)banner adImpressionData:(HZAdImpressionData *)adImpressionData;
...
/**
 *  Fetches a banner and places it in the view. This is a simplest way to use banners.
 *
 ...
 *  @param delegate   The delegate for the banner ad.
...
+ (void)placeBannerInView:(UIView *)view
                 ...
                 delegate:(id<HZBannerAdDelegate>)delegate
 ...
 *  @param success    A block called if we fetch a banner.
 *  @param failure    A block called if we fail to fetch a banner.
 *  @deprecated Use `placeBannerInView:position:options:delegate:success:failure` instead.
 */
+ (void)placeBannerInView:(UIView *)view
                 position:(HZBannerPosition)position
                  options:(HZBannerAdOptions *)options
 ...
 *  @param success A block called if we fetch a banner.
 *  @param failure A block called if we fail to fetch a banner.
 *  @deprecated Use `requestBannerWithOptions:delegate:success:failure` instead.
 */
+ (void)requestBannerWithOptions:(HZBannerAdOptions *)options
                         success:(void (^)(HZBannerAd *banner))success
                         failure:(void (^)(NSError *error))failure DEPRECATED_ATTRIBUTE;
